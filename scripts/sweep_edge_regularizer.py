"""Run diagnostic oracle edge-KL regularizer sweeps for ELBO training."""
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="outputs/linear_gaussian_edge_regularizer")
    ...
    if base_config["model"] != "elbo_edge_mlp":
        raise ValueError("edge regularizer sweep requires model: elbo_edge_mlp")
    ...
    for weight in weights:
        for seed in seeds:
            ...
            rows.append(_load_run(run_dir, seed=seed, edge_kl_weight=weight))
...
def _make_config(
    ...
    *,
    ...
    return {
        **base_config,
        "name": f"linear_gaussian_elbo_edge_mlp_edge_kl_{edge_kl_weight:g}_seed_{seed}",
        "seed": seed,
...
def _load_run(run_dir: Path, *, seed: int, edge_kl_weight: float) -> Row:
...
def _aggregate(rows: list[Row]) -> list[dict[str, float | int]]:
    ...
    for weight in sorted({row.edge_kl_weight for row in rows}):
        grouped = [row for row in rows if row.edge_kl_weight == weight]
        item: dict[str, float | int] = {
            "edge_kl_weight": weight,
...
def _render_report(summary: list[dict[str, float | int]], rows: list[Row]) -> str:
    lines = [
        ...
        "",
        "| edge KL weight | Seeds | filter KL | edge KL | state RMSE global | state NLL | cov 90 | var ratio | pred NLL | ELBO | oracle ELBO |",
        "|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for item in summary:
        lines.append(
            "| {edge_kl_weight:g} | {num_seeds} | {filter_kl_mean:.6f} +/- "
    ...
    lines.extend(
        [
            "",
            "This is a diagnostic linear-Gaussian sweep because nonzero weights use the oracle edge posterior.",
            "",
            "## Per-Seed Rows",
            "",
            "| Seed | edge KL weight | filter KL | edge KL | state RMSE global | state RMSE time mean | state NLL | cov 90 | var ratio | pred NLL | ELBO | oracle ELBO |",
            "|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
        ]
    )
    for row in rows:
