"""Run teacher-forced vs self-fed supervised vs ELBO budget sweeps."""
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="outputs/linear_gaussian_self_fed_supervised")
    ...
    run_specs = [
        ("teacher-forced supervised", Path(args.teacher_forced_config)),
        ("self-fed supervised", Path(args.self_fed_config)),
        ("ELBO", Path(args.elbo_config)),
    ]
    ...
    for label, config_path in run_specs:
        ...
        for steps in step_counts:
            for seed in seeds:
                ...
                rows.append(_load_run(run_dir, seed=seed, model=label, steps=steps))
...
def _render_report(summary: list[dict[str, float | int | str]], rows: list[Row]) -> str:
    lines = [
        "# Linear-Gaussian Self-Fed Supervised Sweep",
        "",
