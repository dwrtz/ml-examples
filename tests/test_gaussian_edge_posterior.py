...
def test_gaussian_edge_log_prob_matches_bivariate_normal() -> None:
    edge_mean = jnp.array([[0.5, -0.2], [1.0, 0.8]], dtype=DEFAULT_DTYPE)
    edge_cov = jnp.array(
        [
            [[0.7, 0.2], [0.2, 0.5]],
            [[1.5, -0.4], [-0.4, 0.9]],
    ...
    z = jnp.array([[0.1, -0.4], [1.3, 0.2]], dtype=DEFAULT_DTYPE)
    ...
    np.testing.assert_allclose(np.asarray(actual), np.asarray(expected), atol=FLOAT_ATOL)


def test_gaussian_edge_sampling_shapes() -> None:
    edge_mean = jnp.zeros((2, 4, 2), dtype=DEFAULT_DTYPE)
    edge_cov = jnp.broadcast_to(
        jnp.array([[1.0, 0.25], [0.25, 0.5]], dtype=DEFAULT_DTYPE),
        (2, 4, 2, 2),
    )
    q_edge = GaussianEdgePosterior.from_mean_cov(edge_mean, edge_cov)
...
def test_k1_mixture_edge_matches_gaussian_edge() -> None:
    ...
    np.testing.assert_allclose(np.asarray(mix_mean), np.asarray(q_edge.joint_mean()), atol=FLOAT_ATOL)
    np.testing.assert_allclose(np.asarray(mix_cov), np.asarray(q_edge.joint_cov()), atol=FLOAT_ATOL)


def test_mixture_belief_weights_normalize_and_moments() -> None:
    belief = GaussianMixtureBelief(
        weights=jnp.array([[0.25, 0.75]], dtype=DEFAULT_DTYPE),
        mean=jnp.array([[0.0, 2.0]], dtype=DEFAULT_DTYPE),
        var=jnp.array([[1.0, 3.0]], dtype=DEFAULT_DTYPE),
    )
...
def test_mixture_edge_log_prob_and_sampling_shapes() -> None:
    q_mix = GaussianMixtureEdgePosterior(
        q_filter=GaussianMixtureBelief(
            weights=jnp.array([[0.4, 0.6], [0.2, 0.8]], dtype=DEFAULT_DTYPE),
            mean=jnp.array([[-1.0, 1.0], [0.0, 2.0]], dtype=DEFAULT_DTYPE),
            var=jnp.array([[0.5, 0.7], [1.0, 1.5]], dtype=DEFAULT_DTYPE),
        ),
        q_backward=ConditionalGaussianMixtureBackward(
            a=jnp.array([[0.1, 0.2], [0.3, 0.4]], dtype=DEFAULT_DTYPE),
            b=jnp.array([[0.0, 0.5], [-0.5, 0.2]], dtype=DEFAULT_DTYPE),
            var=jnp.array([[0.8, 0.9], [1.1, 1.2]], dtype=DEFAULT_DTYPE),
        ),
    )
    z_t = jnp.array([0.2, 1.1], dtype=DEFAULT_DTYPE)
    z_tm1 = jnp.array([-0.1, 0.4], dtype=DEFAULT_DTYPE)
