...
def test_direct_predictive_head_outputs_do_not_depend_on_current_y() -> None:
    state_params = LinearGaussianParams(q=0.1, r=0.1, m0=1.0, p0=10.0)
    batch = make_linear_gaussian_batch(
        LinearGaussianDataConfig(batch_size=3, time_steps=5),
        state_params,
    ...
    outputs = run_direct_predictive_mlp_head(params, prev_mean, prev_var, batch.x, state_params)
    changed_y_batch = replace(batch, y=batch.y - 1000.0)
    changed_outputs = run_direct_predictive_mlp_head(
        params,
        prev_mean,
        prev_var,
        changed_y_batch.x,
